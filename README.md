...
* **Protección contra fuerza bruta en OTP, sanitización y SEO estructurado (2026-09-12):** Conteo de intentos fallidos en OTP (bloqueo al 5.° intento en `User` y `app.controller`), comparación timing-safe (`crypto.timingSafeEqual`), sanitización de códigos OTP en logs de producción, rate limiting en registro de comercios (`leadLimiter` 10/15min en `/app/api/lead`), validación de expiración en canjes (`redemption.controller`) y datos estructurados JSON-LD Schema.org agregados a `/usuarios/` y `/negocios/` para validación en Google Rich Results y Facebook Debugger.
